...
/*
 ...
 In addition to dragging a cell, WZHReorderTableView will
...
@property (nonatomic,retain) UITableViewCell *draggedCell;
@property (nonatomic,retain) NSIndexPath *indexPathBelowDraggedCell;
@property (nonatomic,retain) CADisplayLink *timerToAutoscroll;

@end

@implementation WZHReorderTableView
@synthesize delegate = aDelegate;
@synthesize dataSource = aDataSource;
@synthesize reorderingEnabled = _reorderingEnabled;
@synthesize draggedCell = _draggedCell;
@synthesize indexPathBelowDraggedCell = _indexPathBelowDraggedCell;
@synthesize timerToAutoscroll = _timerToAutoscroll;
- (void)dealloc {
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
         ...
         */
        WZHReorderTableView *blockSelf = self;
        if (!resignActiveObserver) {
            resignActiveObserver = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationWillResignActiveNotification object:nil queue:nil usingBlock:^(NSNotification *arg1) {
                if ([blockSelf isDraggingCell]) {
                    WZHReorderTableView *strongBlockSelf = blockSelf;
                    [strongBlockSelf retain];
...
- (void)setDataSource:(id<WZHReorderTableViewDataSource>)newDataSource {
    [super setDataSource:newDataSource];
    
    aDataSource = newDataSource;
}
- (id)dataSource {
    return [super dataSource];
}
- (void)setDelegate:(id<WZHReorderTableViewDelegate>)newDelegate {
    [super setDelegate:newDelegate];
    
    aDelegate = newDelegate;
}
- (id)delegate {
    return [super delegate];
}
/*
 *	Initializes gesture recognizers and adds them to self
 */
- (void)establishGestures {
	if (!self)
		return;
	
	if (!longPressGestureRecognizer || ![self.gestureRecognizers containsObject:longPressGestureRecognizer]) {
	...
	if (!dragGestureRecognizer || ![self.gestureRecognizers containsObject:dragGestureRecognizer]) {
...
- (void)setReorderingEnabled:(BOOL)reorderingEnabled {
    if (_reorderingEnabled == reorderingEnabled) {
        return;
    }
...
- (void)longPressRecognized {
	...
	if (indexPathOfSomeOtherRow != nil)
		[self reloadRowsAtIndexPaths:[NSArray arrayWithObject:indexPathOfSomeOtherRow] withRowAnimation:UITableViewRowAnimationNone];
	...
	[UIView animateWithDuration:0.25 delay:0 options:(UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionCurveEaseInOut) animations:^{
		[self.dataSource reorderTableView:self addDraggableIndicatorsToCell:self.draggedCell forIndexPath:indexPathOfRow];
	...
	if ([self.delegate respondsToSelector:@selector(reorderTableView:didBeginDraggingAtRow:)])
		[self.delegate reorderTableView:self didBeginDraggingAtRow:indexPathOfRow];
...
- (void)fireAutoscrollTimer:(CADisplayLink *)sender {
	...
	if (blankCell && blankCell.hidden == NO) {
	...
	if (autoscrollOption == AutoscrollStatusCellAtTop) {
		...
		_draggedCell.center = CGPointMake(_draggedCell.center.x, _draggedCell.center.y - scrollDistance);
		...
		_draggedCell.center = CGPointMake(_draggedCell.center.x, _draggedCell.center.y + autoscrollDistance);
	}
 ...
 */
- (BOOL)touchCanceledAfterDragGestureEstablishedButBeforeDragging {
	return (self.draggedCell && longPressGestureRecognizer.state == UIGestureRecognizerStateEnded && dragGestureRecognizer.state == UIGestureRecognizerStateFailed);
}
...
- (UITableViewCell *)cellPreparedToAnimateAroundAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([self.dataSource respondsToSelector:@selector(cellIdenticalToCellAtIndexPath:forReorderTableView:)]) {
        cellCopy = [self.dataSource cellIdenticalToCellAtIndexPath:indexPath forReorderTableView:self];
    }
	else {
...
- (void)keepDraggedCellVisible {
	 ...
	 */
	if (_draggedCell.frame.origin.y <= 0) {
		CGRect newDraggedCellFrame = _draggedCell.frame;
		newDraggedCellFrame.origin.y = 0;
		_draggedCell.frame = newDraggedCellFrame;
	 ...
	 */
	CGFloat maxYOffsetOfDraggedCell = contentRect.origin.x + contentRect.size.height - _draggedCell.frame.size.height;
    
	if (_draggedCell.frame.origin.y >= maxYOffsetOfDraggedCell) {
		CGRect newDraggedCellFrame = _draggedCell.frame;
...
- (void)updateFrameOfDraggedCellForTranlationPoint:(CGPoint)translation {
	...
	CGPoint newDraggedCellCenter = {
		.x = _draggedCell.center.x,
		.y = newYCenter
	};
    
	_draggedCell.center = newDraggedCellCenter;
...
- (void)endedDragGestureWithTranslationPoint:(CGPoint)translation {
	 ...
	 */
	if ([self.delegate respondsToSelector:@selector(reorderTableView:willEndDraggingToRow:)])
		[self.delegate reorderTableView:self willEndDraggingToRow:self.indexPathBelowDraggedCell];
	...
	if ([self.delegate respondsToSelector:@selector(reorderTableView:shouldHideDraggableIndicatorForDraggingToRow:)]) {
		hideDragIndicator = [self.delegate reorderTableView:self shouldHideDraggableIndicatorForDraggingToRow:blankIndexPath];
    }
	/*
	...
	[UIView animateWithDuration:0.25 delay:0 options:(UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState) animations:^{
		 ...
		 */
		if (hideDragIndicator) {
			[self.dataSource reorderTableView:self hideDraggableIndicatorsOfCell:oldDraggedCell];
        }
	} completion:^(BOOL finished) {
		/*
		 ...
		 */
		[self.dataSource reorderTableView:self removeDraggableIndicatorsFromCell:oldDraggedCell];
        
		[oldDraggedCell removeFromSuperview];
        
		if( [self.delegate respondsToSelector:@selector(reorderTableView:didEndDraggingToRow:)] )
			[self.delegate reorderTableView:self didEndDraggingToRow:blankIndexPath];
...
- (void)fastCompleteGesturesWithTranslationPoint:(CGPoint) translation {
	...
	[self.dataSource reorderTableView:self removeDraggableIndicatorsFromCell:self.draggedCell];
...
- (void)shuffleCellsOutOfWayOfDraggedCellIfNeeded {
	 ...
	 */
	if (_draggedCell.center.y < CGRectGetMidY(rectOfCoveredCells)) {
		/*
	 ...
	 */
	if (rowToMoveTo && !(rowToMoveTo.section == self.indexPathBelowDraggedCell.section && rowToMoveTo.row == self.indexPathBelowDraggedCell.row)) {
		/*
		 ...
		 */
		NSIndexPath *formerBlankIndexPath = [NSIndexPath indexPathForRow:_indexPathBelowDraggedCell.row inSection:_indexPathBelowDraggedCell.section];
...
- (NSArray *)addShadowViewsToCell:(UITableViewCell *)selectedCell {
	 ...
	 */
	if (!selectedCell.selectedBackgroundView) {
		return nil;
    }
